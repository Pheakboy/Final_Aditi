...
const typeConfig = {
  LOAN_APPROVED: { label: "Loan Approved", color: "bg-emerald-100 text-emerald-700 border-emerald-200" },
  LOAN_REJECTED: { label: "Loan Rejected", color: "bg-red-100 text-red-700 border-red-200" },
  BROADCAST: { label: "Broadcast", color: "bg-sky-100 text-sky-700 border-sky-200" },
        ...
        <div className="bg-white rounded-2xl card-shadow overflow-hidden">
          {dataLoading ? (
            ...
            <div className="p-12 text-center">
              ...
              <p className="text-slate-400 text-xs mt-1">You&apos;ll be notified when your loan status changes</p>
            </div>
          ) : (
            <div className="divide-y divide-slate-50">
              {notifications.map((notif) => {
                ...
                return (
                  <div key={notif.id} className={`flex items-start gap-4 px-6 py-4 transition-colors ${!notif.isRead ? "bg-teal-50/40" : "hover:bg-slate-50"}`}>
                    {/* Unread dot */}
