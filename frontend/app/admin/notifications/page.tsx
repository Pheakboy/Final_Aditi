...
export default function AdminNotificationsPage() {
  ...
  useEffect(() => {
    if (user && isAdmin) fetchNotifs(0);
  }, [user, isAdmin, fetchNotifs]);
  ...
  const handleSendToUser = async (e: React.FormEvent) => {
    ...
    setUserSuccess("");
    try {
      await adminApi.sendNotificationToUser(userForm.userId, {
        title: userForm.title,
        message: userForm.message,
      });
      setUserSuccess(
        `Notification sent to user #${userForm.userId} successfully.`,
      );
  ...
  return (
    <AdminLayout
      title="Notifications"
      subtitle="Send messages to users and manage notifications"
    >
      <div className="p-6 lg:p-8">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-10">
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm">
            ...
            {broadcastSuccess && (
              <div className="bg-emerald-50 border border-emerald-200 text-emerald-700 px-3 py-2 rounded-xl text-sm mb-4">
                {broadcastSuccess}
              </div>
            )}
            {broadcastError && (
            ...
            <form onSubmit={handleBroadcast} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  required
                  className="w-full px-4 py-2.5 bg-white border border-slate-200 rounded-xl text-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500"
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={4}
                  required
                  className="w-full px-4 py-2.5 bg-white border border-slate-200 rounded-xl text-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 resize-none"
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm">
            <div className="flex items-center gap-3 mb-5">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500">
                  Targeted notification by User ID
                </p>
              </div>
            </div>
            {userSuccess && (
        ...
        <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm">
          <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
            ...
            <button
              onClick={() => fetchNotifs(currentPage)}
                  ...
                  >
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        <span
                          className={`text-xs font-medium px-2 py-0.5 rounded-full ${typeColor[n.type] ?? typeColor.GENERAL}`}
                        >
                          {n.type}
                        </span>
                        <span
                          className={`text-xs px-1.5 py-0.5 rounded ${n.isRead ? "text-slate-400" : "text-amber-600 bg-amber-50"}`}
                        >
                      ...
                      <p className="text-xs text-slate-400 mt-1">
                        To: {n.recipientUsername} ({n.recipientEmail})
                      </p>
                    </div>
                    <button
                      onClick={() => setDeleteId(n.id)}
                      className="shrink-0 text-xs text-red-500 hover:text-red-700 font-medium mt-1 transition-colors"
                    >
                      Delete
                    </button>
                  </div>
              ...
              {totalPages > 1 && (
                <div className="flex items-center justify-between px-6 py-4 border-t border-slate-100">
                  ...
                  <div className="flex gap-2">
                    <button
                      onClick={() => fetchNotifs(currentPage - 1)}
                      disabled={currentPage === 0}
                      className="px-3 py-1.5 text-xs border border-slate-200 rounded-lg bg-white text-slate-600 hover:bg-slate-50 disabled:opacity-40 transition-colors"
                    >
                      Prev
                    </button>
                    <button
      ...
      {deleteId && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white border border-slate-200 rounded-2xl shadow-xl p-6 w-full max-w-sm mx-4">
            <h3 className="text-base font-semibold text-slate-900 mb-2">
              Delete Notification?
            </h3>
            <p className="text-sm text-slate-500 mb-5">
              This will permanently remove the notification from the
              recipient&apos;s inbox.
            </p>
            <div className="flex gap-3">
              ...
              <button
                ...
                className="flex-1 py-2 bg-slate-100 text-slate-700 text-sm font-semibold rounded-xl hover:bg-slate-200 transition-colors"
              >
                Cancel
              </button>
            </div>
