...
export default function AdminUserDetailPage() {
  ...
  useEffect(() => {
    if (user && isAdmin && userId) fetchProfile();
  }, [user, isAdmin, userId]);
  ...
  return (
    <AdminLayout
      title="User Detail"
      subtitle="Full profile and financial overview"
      mainBg="bg-slate-900"
    >
      <div className="p-6 lg:p-8">
        <div className="animate-fade-in">
          ...
          <p className="text-slate-400 text-sm mb-8">
            Full profile and financial overview
          </p>
        </div>

        {error && (
          <div className="bg-red-900/30 border border-red-700/50 text-red-300 px-4 py-3 rounded-xl text-sm mb-6">
            {error}
          </div>
        )}
        {success && (
          <div className="bg-emerald-900/30 border border-emerald-700/50 text-emerald-300 px-4 py-3 rounded-xl text-sm mb-6">
            {success}
          </div>
        )}
          ...
          <div className="space-y-6">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <div className="flex items-center gap-3">
                      ...
                      <span
                        className={`text-xs px-2 py-0.5 rounded-full font-medium ${profile.enabled ? "bg-emerald-900/50 text-emerald-300" : "bg-red-900/50 text-red-300"}`}
                      >
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              {[
                {
                  label: "Total Income",
                  value: formatCurrency(profile.totalIncome),
                  color: "text-emerald-400",
                },
                {
                  label: "Total Expenses",
                  value: formatCurrency(profile.totalExpenses),
                  color: "text-red-400",
                },
                {
                  label: "Savings Balance",
                  value: formatCurrency(profile.savingsBalance),
                  color:
                ...
                {
                  label: "Risk Level",
                  value: profile.latestRiskLevel ?? "N/A",
                  color: "text-white",
                },
              ].map((s) => (
                <div
                    ...
                    <tbody className="divide-y divide-slate-700/50">
                      {profile.loans?.map((loan: Loan) => (
                        ...
                        >
                          ...
                          <td className="px-5 py-3">
                            <span
                              className={`text-xs font-semibold px-2.5 py-1 rounded-full ${loan.status === "APPROVED" ? "bg-emerald-900/50 text-emerald-300" : loan.status === "REJECTED" ? "bg-red-900/50 text-red-300" : "bg-amber-900/50 text-amber-300"}`}
                            >
                              {loan.status}
                            </span>
                          </td>
                ...
                {profile.recentTransactions?.length === 0 ? (
                  ...
                  <div className="divide-y divide-slate-700/50">
                    {profile.recentTransactions?.map((tx: Transaction) => (
                      <div
                        key={tx.id}
                        className="flex items-center gap-4 px-6 py-3 hover:bg-slate-700/30 transition-colors"
                      >
                        <div
                          className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${tx.type === "INCOME" ? "bg-emerald-900/50" : "bg-red-900/50"}`}
                        >
                          <svg
                            className={`w-4 h-4 ${tx.type === "INCOME" ? "text-emerald-400" : "text-red-400"}`}
                            fill="none"
                            viewBox="0 0 24 24"
                            stroke="currentColor"
                          >
