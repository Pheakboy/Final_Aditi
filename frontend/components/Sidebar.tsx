...
// ─── User Sidebar (clean white + teal theme) ──────────────────────────────────
  ...
  return (
    <aside className="w-64 bg-white border-r border-slate-100 min-h-screen flex flex-col shadow-sm">
      ...
      <nav className="flex-1 px-3 pb-3 space-y-0.5">
        <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider px-3 pb-1 pt-2">
          Menu
        </p>
        {navItems.map((item) => {
...
// ─── Admin Sidebar (dark slate + indigo theme) ───────────────────────────────
function AdminSidebar() {
  ...
  return (
    <aside className="w-64 bg-slate-900 min-h-screen flex flex-col admin-scroll">
      ...
      <div className="p-4">
        <div className="bg-slate-800 rounded-xl p-3 border border-slate-700">
          <div className="flex items-center gap-3">
            ...
            <span className="text-xs bg-indigo-500/20 text-indigo-300 border border-indigo-500/30 px-1.5 py-0.5 rounded font-medium shrink-0">
              Admin
            </span>
          </div>
        </div>
      ...
      {/* Navigation */}
      <nav className="flex-1 px-3 pb-3 space-y-0.5">
        <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider px-3 pb-1 pt-2">
          Management
        </p>
        {navItems.map((item) => {
          const isActive = pathname === item.href;
...
// ─── Main export: picks the right sidebar ────────────────────────────────────
