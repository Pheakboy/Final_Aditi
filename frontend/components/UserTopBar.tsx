...
function notifMeta(type: string): { bg: string } {
  switch (type) {
    case "LOAN_APPROVED":
      return { bg: "bg-emerald-100 text-emerald-600" };
    case "LOAN_REJECTED":
      return { bg: "bg-red-100 text-red-600" };
    case "BROADCAST":
      return { bg: "bg-sky-100 text-sky-600" };
    default:
  ...
  return (
    <header className="h-16 shrink-0 bg-white border-b border-slate-200 px-5 flex items-center justify-between shadow-sm z-30 relative">
      ...
      <div className="flex items-center gap-1.5 shrink-0">
        {/* Greeting pill (hidden on mobile) */}
        <span className="hidden md:inline-flex items-center gap-1.5 text-[12px] font-semibold text-slate-500 bg-slate-50 border border-slate-200 px-3 py-1.5 rounded-lg mr-1">
          {greeting},&nbsp;
          <span className="text-teal-600 font-bold">{user?.username}</span>
        </span>
        ...
        {onRefresh && (
          <button
            onClick={onRefresh}
            className="w-8 h-8 rounded-lg border border-slate-200 bg-slate-50 flex items-center justify-center text-slate-400 hover:text-teal-600 hover:border-teal-200 hover:bg-teal-50 transition-all"
            title="Refresh data"
          >
            <svg
              className="w-3.5 h-3.5"
        ...
        <div ref={notifRef} className="relative">
          <button
            ...
            className="relative w-8 h-8 rounded-lg border border-slate-200 bg-slate-50 flex items-center justify-center text-slate-400 hover:text-teal-600 hover:border-teal-200 hover:bg-teal-50 transition-all"
            title="Notifications"
          >
            <svg
              className="w-4 h-4"
          ...
          {notifOpen && (
            <div className="absolute right-0 top-11 w-80 bg-white rounded-2xl shadow-2xl border border-slate-200 z-50 overflow-hidden animate-slide-up">
              ...
              <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100">
                ...
                {unread > 0 && (
                  <button
                    ...
                    className="text-[11px] font-semibold text-teal-500 hover:text-teal-700 disabled:opacity-50 transition-colors"
                  >
                  ...
                  notifications.map((notif) => {
                    ...
                    return (
                      <button
                        ...
                        className={`w-full flex items-start gap-3 px-4 py-3 text-left hover:bg-slate-50 transition-colors border-b border-slate-50 last:border-0 ${!notif.isRead ? "bg-teal-50/40" : ""}`}
                      >
                        <div
              ...
              <Link
                ...
                className="flex items-center justify-center gap-1 py-3 text-[12px] font-semibold text-teal-600 hover:text-teal-800 border-t border-slate-100 hover:bg-teal-50/50 transition-colors"
              >
        ...
        <div ref={profileRef} className="relative">
          ...
          >
            {/* Avatar */}
            <div className="relative w-8 h-8 rounded-full bg-linear-to-br from-teal-500 to-sky-500 flex items-center justify-center shadow-sm shrink-0 ring-2 ring-white">
            ...
            <div className="hidden sm:block text-left">
              ...
              <span className="text-[9px] font-bold text-teal-600 bg-teal-50 px-1.5 py-0.5 rounded-md leading-none mt-0.5 inline-block">
                MEMBER
              </span>
            </div>
          ...
          {profileOpen && (
            <div className="absolute right-0 top-12 w-60 bg-white rounded-2xl shadow-2xl border border-slate-200 z-50 overflow-hidden animate-slide-up">
              {/* User info */}
              <div className="p-4 bg-linear-to-br from-teal-50 via-slate-50 to-white border-b border-slate-100">
                <div className="flex items-center gap-3">
                  <div className="relative w-11 h-11 rounded-full bg-linear-to-br from-teal-500 to-sky-500 flex items-center justify-center shadow-md shrink-0">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <span className="inline-flex items-center gap-1 text-[9px] font-black text-teal-600 bg-teal-100 px-1.5 py-0.5 rounded-md mt-1">
                      <svg
                        className="w-2 h-2"
                        fill="currentColor"
                        viewBox="0 0 20 20"
